---
title: "Previous_Day_High_Shot_Chart"
...
library(tidyverse)
devtools::install_github("abresler/nbastatR") # NBA DATA
library("nbastatR")
...
plot_court = function(court_theme = court_themes$paper, use_short_three = FALSE) {
  if (use_short_three) {
...
comps <- strsplit(as.character(yesterdays_date), "-")
print(comps)
newDate <- paste(comps, "")
print(newDate)

# Convert today's date to simple string (I'm sure there are better ways of doing this, but this worked for me)
concatDate <- sapply(newDate, function(x) paste(x[1:2], collapse=""))
print(concatDate)
finalDate <- gsub("[^0-9.-]", "", concatDate)
print(finalDate)

print(as.numeric(finalDate))
...
nbaTeamDict <- nba_teams() %>% filter(isNonNBATeam == 0)

# Grab team shot data
# team_names <- c("Atlanta Hawks", "Boston Celtics", "Brooklyn Nets", "Charlotte Hornets", "Chicago Bulls", "Cleveland Cavaliers",
...
all_shots <- teams_shots(all_active_teams = T, seasons = 2022, season_types = "Regular Season", date_from = as.numeric(finalDate), date_to = as.numeric(finalDate))
yesterday_shots <- all_shots %>% filter(dateGame == as.numeric(finalDate))
...
# NBA Chart
```{r}
p1 <- plot_court(court_themes$paper, use_short_three = F) +
  geom_point(data = final_durant, aes(x = x, y = y, color = final_durant$isShotMade, fill = final_durant$isShotMade), 
             size =3, shape = 21, stroke = .8) +  
  # below line is made shots display
  scale_color_manual(values = c("green4","red4"), aesthetics = "color", breaks=c("TRUE", "FALSE"), labels=c("Made", "Missed")) +
  # below line is missed shots display
  ...
  theme(plot.title = element_text(hjust = .5, size = 22, family = "Comic Sans MS", face = "bold", vjust = -4),
        ...
        legend.text = element_text(hjust = .5, size = 10, family = "Comic Sans MS", face = "bold", colour = "#2c3a5c"),
        plot.caption = element_text(hjust = .5, size = 6, family = "Comic Sans MS", face = "bold", colour = "#2c3a5c", vjust = 8)) +
  # Title
  ggtitle(label = "Kevin Durant vs. Milwaukee",
          subtitle = "49 PTS | 17 REB | 10 AST | 4-9 3PT - 6/15/21") 
# + labs(caption = "Tutorial: @DSamangy")

ggdraw(p1) + theme(plot.background = element_rect(fill="cornsilk", color = NA)) 
